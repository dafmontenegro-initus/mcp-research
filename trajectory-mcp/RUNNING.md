...
The server requires a Bearer token on every connection. Unknown tokens are rejected.

Users are stored in `users.json` (not committed to git). Use `manage_users.py` to manage them.
...
# Add a new user (generates a token automatically)
python3 manage_users.py add john.doe@trajectoryinc.com

# Show a user's token (to share with them)
python3 manage_users.py token john.doe@trajectoryinc.com

# Revoke access immediately
python3 manage_users.py remove john.doe@trajectoryinc.com
```

When you add a user, the script prints the exact line they need to add to their Claude Desktop config. Restart the server after any change to `users.json`.
  ...
  "mcpServers": {
    "Trajectory": {
      ...
      "args": [
        "-y",
        "mcp-remote",
        "http://64.137.145.121:8080/mcp?token=<your-token>",
        "--allow-http"
      ]
    }
  }
...
Replace `<your-token>` with the token generated by `manage_users.py add <your-email>`.
Example: `http://64.137.145.121:8080/mcp?token=traj_UiC9JY734KXktXr5npou_A`
...
**Connections without a valid token are rejected.** The error appears in the MCP server log
as `[auth] Rejected — ...`.
...
Every tool call is recorded in `traces/traces.db` (SQLite, not committed to git).
Each authenticated user gets their own sessions. A new session starts after 30 minutes
of inactivity.

```bash
# Summary via QA script (recommended)
python3 tests/qa_demo.py NWN

# Quick raw query
python3 -c "
import sqlite3
conn = sqlite3.connect('traces/traces.db')
conn.row_factory = sqlite3.Row
for s in conn.execute('''
    SELECT user_email, company_id,
           datetime(started_at,\"unixepoch\",\"localtime\") start,
...
for c in conn.execute(
    'SELECT datetime(called_at,\"unixepoch\",\"localtime\") t, tool, error_msg '
    'FROM tool_calls WHERE ok=0 AND called_at > ? ORDER BY called_at DESC',
    [time.time() - 604800]
):
    print(dict(c))
"
...
The DB is created automatically on the first tool call. No setup required.
...
| Claude Desktop shows "Server disconnected" immediately | Missing or invalid token — run `manage_users.py token <email>` and add `--header` to config |
| New user can't connect after `manage_users.py add` | Restart the MCP server — `users.json` is read at startup |
