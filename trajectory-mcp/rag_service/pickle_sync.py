...
# Embeddings are regenerated by the daemon at most once per day — 24h TTL is safe.
STALENESS_SECONDS = 86400  # 24 h
...
def ensure_warm(company_id: str, data_type: str, environment: str = "prod", force_check: bool = False) -> dict:
    """
    Lazy-load pickles for (company_id, data_type) from S3 into ChromaDB.

    - On first call for a company: loads all pickles from S3.
    - On subsequent calls: skips files ingested within STALENESS_SECONDS (24h).
    - force_check=True: bypasses the 24h cache and re-checks S3 for every key.
      Used by the periodic re-warm so daemon updates (~every 50 min) are picked up
    ...
    for key in keys:
        ...
        if cached:
            ...
            if cached.get("not_found"):
                ...
                # Skip S3 head call if ingested within the staleness window — UNLESS the
                # caller is the periodic re-warm (force_check=True), which explicitly wants
                # to detect daemon updates without waiting the full 24h TTL.
                if not force_check:
                    ingested_at = cached.get("ingested_at", "")
                    if ingested_at:
                        try:
                            age = (now - datetime.fromisoformat(ingested_at)).total_seconds()
                            if age < STALENESS_SECONDS:
                                continue  # still fresh — no S3 call needed
                        except Exception:
                            pass
                # TTL expired — verify against S3 before deciding to re-ingest
                try:
                    obj_meta = s3.head_object(Bucket=bucket, Key=key)
                    s3_modified = _s3_iso(obj_meta)
                    if cached.get("s3_last_modified") == s3_modified:
                        continue  # unchanged in S3 — skip
                except Exception:
